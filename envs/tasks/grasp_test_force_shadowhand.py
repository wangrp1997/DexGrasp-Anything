...
#########六个方向都要满足
    def push_object(self):
        ...
        achieve_6dir1 = torch.zeros(self.num_envs).bool().to(self.device)
        ...
        with tqdm(range(self.force_num_steps * 6), desc='Test Push Object:') as pbar:
            for i_direction in self.object_force_directions:
                ...
                print(f'achieve_6dir: {achieve_6dir}')
                achieve_6dir1 |= i_achieve
                print(f'achieve_6dir1: {achieve_6dir1}')
        return achieve_6dir, achieve_6dir1

# ##########六个方向中一个满足即可
#     def push_object(self):
#         # self._set_init_pose()
#         self._set_normal_force_pose()
#         achieve_6dir = torch.zeros(self.num_envs).bool().to(self.device)
#         self.force_num_steps = self.cfg['eval_policy']['dynamic']['num_steps']
#         self.object_force_directions = self.cfg['eval_policy']['dynamic']['directions'].values()
#         with tqdm(range(self.force_num_steps * 6), desc='Test Push Object:') as pbar:
#             for i_direction in self.object_force_directions:
    ...
    def _load_obj_asset(self):
        ...
        with tqdm(total=used_len) as pbar:
            ...
            if self.object_name is None:
                ...
                #unidex
                # object_mesh_path = os.path.join('/inspurfs/group/mayuexin/datasets/UniDexGrasp/DFCData/obj_scale_urdf', f'{self.object_name}.obj')
                # object_urdf_path = os.path.join('/inspurfs/group/mayuexin/datasets/UniDexGrasp/DFCData/obj_scale_urdf', f'{self.object_name}.urdf')

                # #dexgrasp
                # object_mesh_path = os.path.join('/inspurfs/group/mayuexin/datasets/DexGraspNet/obj_scale_urdf', f'{self.object_name}.obj')
                # object_urdf_path = os.path.join('/inspurfs/group/mayuexin/datasets/DexGraspNet/obj_scale_urdf', f'{self.object_name}.urdf')
                
                # # #DexGRAB
                # object_mesh_path = os.path.join('/inspurfs/group/mayuexin/datasets/DexGRAB/contact_meshes', f'{self.object_name}.ply')
                # object_urdf_path = os.path.join('/inspurfs/group/mayuexin/datasets/DexGRAB/contact_meshes', f'{self.object_name}.urdf')
