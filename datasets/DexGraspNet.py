...
class DexGraspNet(Dataset):
    ...
    def __init__(self, cfg: DictConfig, phase: str, slurm: bool, case_only: bool=False, **kwargs: Dict) -> None:
        ...
        input_file = os.path.join(cfg.asset_dir,"grasp.json")
    ...
    def _pre_load_data(self, case_only: bool) -> None:
        ...
        self.frames = []
        self.scene_pcds = {}        
        grasp_dataset = torch.load(os.path.join(self.data_dir, 'dexgraspnet_shadowhand_downsample.pt'))
        self.scene_pcds = pickle.load(open(self.scene_path, 'rb'))
        self.dataset_info = grasp_dataset['info'] 
        if self.use_llm:
            # Getting descriptions from LLM
            scene_text_file = os.path.join(self.data_dir, "DexGraspnet_gpt4o_mini.json")
...
if __name__ == '__main__':
    ...
    dataloader = DexGraspNet(cfg.dataset, 'train', False).get_dataloader(batch_size=4,
