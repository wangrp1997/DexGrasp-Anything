...
class real_dex(Dataset):
    ...
    def _pre_load_data(self, case_only: bool) -> None:
        ...
        grasp_dataset = torch.load(os.path.join(self.data_dir, 'realdex_shadowhand_downsample.pt' if self.is_downsample else 'realdex_shadowhand.pt'))
        self.scene_pcds = pickle.load(open(self.scene_path, 'rb'))
        self.dataset_info = grasp_dataset['info']

        if self.use_llm:
            # Getting descriptions from LLM
            scene_text_file = os.path.join(self.asset_dir, "Realdex_gpt4o_mini.json")
